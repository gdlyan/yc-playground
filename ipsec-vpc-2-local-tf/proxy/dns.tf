...
resource "yandex_dns_recordset" "a_record_onvpn_nocodb" {
  zone_id = data.yandex_dns_zone.public_zone.id
  name    = "${var.subdomain}.onvpn.${var.domain}."
  ttl     = 600
  type    = "A"
  data    = [yandex_compute_instance.proxy_docker_instance.network_interface.0.ip_address]
}
